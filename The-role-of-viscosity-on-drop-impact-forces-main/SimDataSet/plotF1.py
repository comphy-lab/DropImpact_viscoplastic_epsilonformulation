...
for oh, df in dataframes.items():
    if "We" in df.columns and "F1" in df.columns:
        plt.plot(df["We"], df["F1"], marker='s', linestyle='-', label=f"Oh={oh}")
plt.xscale("log")  # 设置 x 轴为对数坐标
plt.xlabel("We (log scale)")
plt.ylabel("F1")
plt.title("F1 vs We for Different Oh")
plt.legend()
plt.grid(True, which="both", linestyle="--", linewidth=0.5)
plt.savefig("F1_We_plot.png", dpi=300)  # 保存图片
plt.show()
