...
#define Xdist (8)
double R_y;
#define R2Drop(x, y) (sq(x - Xdist) + sq(y/R_y))
...
event adapt(i++)
{
  ...
  else
  {
    ...
    adapt_wavelet((scalar *){f, KAPPA,u.x, u.y, D2c}, (double[]){fErr, KErr,VelErr, VelErr, DissErr}, MAXlevel, MINlevel);
  }
...
event postProcess(t += tsnap)
{
  ...
  double vcm=0;
  foreach (reduction(+ : ke) reduction(+ : vcm) reduction(min : xMin))
  {
