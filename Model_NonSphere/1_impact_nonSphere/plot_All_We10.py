...
for variable in variables:
    ...
    contactTime=0
    ratioBounce=0       
    ratio=(2*df["Rmax"]/(df["Zmax"]-df["Zmin"])) 
    mask0 = (t >= 1.5) & (df["Zmin"] > 0.005)
    if mask0.empty:
        vcbounce=vc1[mask0].iloc[0]
        contactTime=t[mask0].iloc[0]        
        ratioBounce=ratio[mask0].iloc[0]
    ...
    data[variable] = {"t": t, "vcm1": vcm1, "dvcm1_dt": dvcm1_dt, "uH": uH, "Zmax":Zmax, "vR": vR, "Rmax":Rmax,"ratio":ratio,"ratioBounce":ratioBounce,"vcbounce":vcbounce,"contactTime":contactTime,"Zmin":Zmin, "vc1":vc1,"F1_t":[F1,F1_time], "F2_t":[F2,F2_time], "uHMAX_t":[uHMAX,uHMAX_t], "vRMAX_t":[vRMAX,vRMAX_t], "RmaxMAX_t":[RmaxMAX,RmaxMAX_t], "ke":ke}
...
# Plot vc-t
labels_vc1 = {variable:(data[variable]["t"], data[variable]["vc1"]) for variable in data}
plot_figure(
    x=None, y=None, labels=labels_vc1,
    xlabel="t", ylabel="vc1",
    title=f"vc1-t for Different {variable_name}",
    save_as=os.path.join(plot_folder, f"figure_vc1_vs_t_for_different_{variable_name}.png")
)

# Plot Zmin-t
labels_Zmin = {variable:(data[variable]["t"], data[variable]["Zmin"]) for variable in data}
plot_figure(
    x=None, y=None, labels=labels_Zmin,
    xlabel="t", ylabel="Zmin",
    title=f"Zmin-t for Different {variable_name}",
    xlim=(6.0, 7),
    save_as=os.path.join(plot_folder, f"figure_Zmin_vs_t_for_different_{variable_name}.png")
)
# Plot ratio-t
labels_ratio = {variable:(data[variable]["t"], data[variable]["ratio"]) for variable in data}
plot_figure(
    x=None, y=None, labels=labels_ratio,
    xlabel="t", ylabel="ratio",
    title=f"ratio-t for Different {variable_name}",
    xlim=(5.0, 7),
    ylim=(0, 2),
    save_as=os.path.join(plot_folder, f"figure_ratio_vs_t_for_different_{variable_name}.png")
)
...
# ratioBounce
ratioBounce_values = [data[variable]["ratioBounce"] for variable in data]
plt.figure(figsize=(8, 6))
plt.plot(variables_exist, ratioBounce_values, '^-', label='Rmax', color='green')
plt.xlabel(f'{variable_name}')
plt.ylabel('ratioBounce')
plt.title(f'ratioBounce vs {variable_name}')

plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_folder, f"figure_All_ratioBounce_vs_{variable_name}.png"), dpi=300)
plt.close()

# vcbounce,"contactTime"
vcbounce_values = [data[variable]["vcbounce"] for variable in data]
plt.figure(figsize=(8, 6))
plt.plot(variables_exist, vcbounce_values, '^-', label='Rmax', color='green')
plt.xlabel(f'{variable_name}')
plt.ylabel('vcbounce')
plt.title(f'vcbounce vs {variable_name}')

plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_folder, f"figure_All_vcbounce_vs_{variable_name}.png"), dpi=300)
plt.close()

contactTime_values = [data[variable]["contactTime"] for variable in data]
plt.figure(figsize=(8, 6))
plt.plot(variables_exist, contactTime_values, '^-', label='Rmax', color='green')
plt.xlabel(f'{variable_name}')
plt.ylabel('contactTime')
plt.title(f'contactTime vs {variable_name}')

plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_folder, f"figure_All_contactTime_vs_{variable_name}.png"), dpi=300)
plt.close()
