...
def process_Results(ti, tsnap,Oh,We,J):
    t = tsnap * ti
    ...
    if not os.path.exists(filepath):
        ...
        return None    
    data = run_executable("./getResults",[filepath,Oh,We,J])
...
def getResults(tsnap, tmax, results_csv, Oh, We, J, CPUStoUse):
    ...
    if os.path.exists(results_csv):
        os.remove(results_csv)
    process_func = partial(process_Results, tsnap=tsnap,Oh=Oh,We=We,J=J)
    results = []
    with mp.Pool(processes=CPUStoUse) as pool:
        for res in pool.imap(process_func, range(nsteps + 1)):
            results.append(res)
    # Write results to a CSV file
    header = ["t","n","ke","vcm","vc","vcm1","vc1","Rmax","Zmax","Zmin","vR","uH"]    
...
if __name__ == "__main__":
    ...
    results=getResults(tsnap, tmax, output_results_csv, Oh, We, J,CPUStoUse)
