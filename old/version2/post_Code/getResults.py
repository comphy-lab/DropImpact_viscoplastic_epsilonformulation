...
def plot_forces(results, output_force_fig):
    # Create a figure and axis
    plt.figure(figsize=(10, 6))
    tp = np.array([row[0] for row in results if row is not None])  # Column 1: tp
    pforce = np.array([row[3] for row in results if row is not None])  # Column 20: pforce
...
if __name__ == "__main__":
    ...
    plot_forces(results, output_force_fig)
